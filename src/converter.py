...
class Color:
    ...
    def __init__(
        self, interpolate: bool = True, hd: bool = False, hillshade: bool = False
    ):
    ...
    def normalize(
        self, lower_bound: np.ndarray, upper_bound: np.ndarray, val: np.ndarray
    ):
        return (val - lower_bound) / (upper_bound - lower_bound)

    def blend_color_val(
        self, a: np.ndarray, b: np.ndarray, t: np.ndarray
    ) -> np.ndarray:
    ...
    def get_hypsometric_color(self, elevation: np.ndarray) -> np.ndarray:
        ...
        pos = self.normalize(
            self.schema.stops[bins], self.schema.stops[bins + 1], elevation
        )
        ...
        for i in range(3):
            if self.interpolate:
                hyp[:, :, i] = self.blend_color_val(
                    color1[:, :, i], color2[:, :, i], pos
                )
            else:
                hyp[:, :, i] = color1[:, :, i]
    ...
    def terrarium_to_hypsometric(self, image: Img.Image, zl: int, y: int) -> Img.Image:
        ...
        if self.hillshade:
            ...
            for i in range(3):
                data[:, :, i] = np.minimum(
                    np.maximum(data[:, :, i] + hillshade * 255 * self.intensity, 0), 255
                )
    ...
    def merge_tiles(self):
        ...
        zoom_dirs = sorted(
            zoom_dirs, key=lambda x: int(os.path.splitext(x)[0].split("/")[-1])
        )
    ...
    def merge_single_tile(self, childs: List[Path]):
        ...
        hd_image = np.concatenate(
            (np.concatenate((im1, im2)), np.concatenate((im3, im4))), axis=1
        )
